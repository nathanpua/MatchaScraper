...
# Optional: The desired capacity to scale down to. Defaults to 0.
DESIRED_CAPACITY = int(os.environ.get('DESIRED_CAPACITY', 0))
# Optional: The minimum size of the ASG. Defaults to 0.
MIN_SIZE = int(os.environ.get('MIN_SIZE', 0))
# Optional: The maximum size of the ASG. Defaults to 1 to allow scaling up again.
...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info(f"Successfully scaled down ASG {ASG_NAME}.")
        return {"statusCode": 200, "body": f"ASG {ASG_NAME} scaled down."}
    except Exception as e:
        logger.error(f"Error scaling down ASG {ASG_NAME}: {e}")
        return {"statusCode": 500, "body": f"Error scaling down ASG: {e}"} 